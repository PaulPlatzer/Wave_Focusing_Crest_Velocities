...
# (setting width_choice=5) that produces "eta1",
...
## Call all the files
fnames = glob('Fig5_data/GWP_k0Lf5_phi*.npz') # to open the files created by the user
#fnames = glob('Fig5_article_data/GWP_2D_Lk05_phi*.npz') # to open the files used in the article
fnames.sort()
Nfi=len(fnames)
...
ftsz=26#fontsize
ax.set_title(r'Focusing time, $k_0L_f=5$',fontsize=ftsz)
...
ax.fill_between(t[nstart2+2:nend]*c0*k0/(2*np.pi), np.percentile(ARATIO2,axis=0,q=2.5)[nstart2+2:nend],
                ...
                color='g', alpha=.2, label=r'2.5% to 97.5% percentiles, $t_{start}=-|\tau|$')
ftsz=26 #fontsize
ax.set_title(r'Amplitude, $k_0L_f=5$',fontsize=ftsz)
ax.set_xlabel(r'$t_{end}/T_p$',fontsize=ftsz)
...
ax.set_ylim([4.8,5.2])
ax.set_xlim([-2*np.abs(tau)*c0*k0/(2*np.pi),0])
ax.set_title(r'Group size, $k_0L_f=5$',fontsize=ftsz)
ax.set_xlabel(r'$t_{end}/T_p$',fontsize=ftsz)
