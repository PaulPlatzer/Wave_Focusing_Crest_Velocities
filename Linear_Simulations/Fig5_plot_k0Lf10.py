...
# (setting width_choice=10) that produces "eta1",
...
# Here we simply load the files that were created in "Fig5_data.py",
...
# Median
ax.plot(t[nstart1+2:nend]*c0*k0/(2*np.pi), np.median(TFOC1,axis=0)[nstart1+2:nend]*c0*k0/(2*np.pi), '-g',
...
ftsz=26 #fontsize
ax.set_title(r'Focusing time, $k_0L_f=10$',fontsize=ftsz)
...
ax.set_ylim([-3,3])
ax.set_xlim([-1*np.abs(tau)*c0*k0/(2*np.pi),0])
...
# Median
ax.plot(t[nstart1+2:nend]*c0*k0/(2*np.pi), np.median(ARATIO1,axis=0)[nstart1+2:nend], '-g',
        linewidth=2,label=r'Median of estimations, $t_{start}=-|\tau|$')
# Quantile : 2.5% and 97.5%
ax.fill_between(t[nstart1+2:nend]*c0*k0/(2*np.pi), np.percentile(ARATIO1,axis=0,q=2.5)[nstart1+2:nend],
                ...
                color='g', alpha=.2, label=r'2.5% to 97.5% percentiles, $t_{start}=-|\tau|$')
ftsz=26 #fontsize
ax.set_title(r'Amplitude, $k_0L_f=10$',fontsize=ftsz)
ax.set_xlabel(r'$t_{end}/T_p$',fontsize=ftsz)
...
plt.clf(); fig=plt.figure(); fig.add_axes(); ax=fig.add_subplot(111)
# Objective
ax.plot(t[nstart1:nend]*c0*k0/(2*np.pi),k0*L*np.ones(len(t[nstart1:nend])),'k',label=r'$L_{f}k_0=10$')
# Median
ax.plot(t[nstart1+2:nend]*c0*k0/(2*np.pi), np.median(LL1,axis=0)[nstart1+2:nend]*k0, '-g',
        linewidth=2,label=r'Median of estimations, $t_{start}=-|\tau|$')
# Quantile : 2.5% and 97.5%
...
ax.set_xlim([-1*np.abs(tau)*c0*k0/(2*np.pi),0])
ax.set_title(r'Group size, $k_0L_f=10$',fontsize=ftsz)
ax.set_xlabel(r'$t_{end}/T_p$',fontsize=ftsz)
ax.set_ylabel(r'$k_0L_{f}$',fontsize=ftsz)
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.,fontsize=ftsz)
ax.tick_params(length=7,labelsize=ftsz,direction='out',pad=10)
fig.set_size_inches([8,6])
plt.show()
