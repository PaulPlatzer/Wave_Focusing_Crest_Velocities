...
#---------#
# This piece of code is to be used alongside with "Linear_GWP_k0Lf2.py" that produces "eta1",
...
## Define the function used of the fit
def gradient_fit2( time , t0 , Tau ):# '2' stands for 'second-order' because of the term '-3.25/((...'
...
nstart1=0 #this starts the observation/estimation at t=-4|\tau|
nstart2=85 #this starts the observation/estimation at t=-2|\tau|
nstart3=85+42 #this starts the observation/estimation at t=-|\tau|
...
cov3=np.full((Nt-3,2,2),np.inf)

#apply the method starting at t=-4|\tau|
nstart=nstart1
...
#apply the method starting at t=-2|\tau|
nstart=nstart2
for nend in range(nstart+3,Nt-3):
    popt, pcov = curve_fit(gradient_fit2, t[nstart:nend], c_grad_mes[nstart:nend],p0=[0,10*2*np.pi/(k0*c0)]
                       ,sigma=STD_FIT*np.ones(nend-nstart), bounds=([-np.inf,0],np.inf))
...
## Store the data
np.savez('GWP_k0Lf2_phi%.5f' % phi0,tfoc1=tfoc1,tau_abs1=tau_abs1
         ...
         cov3_00=cov3[:,0,0],cov3_11=cov3[:,1,1],cov3_10=cov3[:,1,0],cov3_01=cov3[:,0,1])